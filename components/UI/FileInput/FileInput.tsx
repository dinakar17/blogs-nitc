...
const FileInput = ({imageUpload}: Props) => {
  return (
    <div className="flex justify-center items-center w-full">
      ...
      >
        ...
        <input id="dropzone-file" type="file" className="hidden" onChange={(e) => { if (e.target.files) imageUpload(e.target.files[0]) }} onDrop={(e) => { if (e.dataTransfer.files) imageUpload(e.dataTransfer.files[0]) }} />
      </label>
    </div>
  );
};
